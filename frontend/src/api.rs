...
pub async fn api_register_user(user_data: &str) -> Result<(), String> {
    let response = match http::Request::post("http://localhost:8000/auth/signup")
        .header("Content-Type", "application/json")
        .body(user_data)
        .send()
        .await
...
pub async fn api_login_user(user_data: &str) -> Result<(), String> {
    let response = match http::Request::post("http://localhost:8000/auth/login")
        .header("Content-Type", "application/json")
        .body(user_data)
        .send()
        .await
...
pub async fn api_fetch_feedbacks((page, limit): (i32, i32)) -> Result<Vec<Feedback>, String> {
    let response = match http::Request::get("http://localhost:8000/users")
        .send()
        .await
    {
        Ok(res) => res,
